...
class buttonNode extends AudioWorkletNode {

    constructor(context, baseURL, options) {
        super(context, 'button', options);
    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, buttonNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel) {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, buttonNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class button {
    ...
    async load() {
        try {
            ...
            let real_url = (this.baseURL === "") ? "button.wasm" : (this.baseURL + "/button.wasm");
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    let re = /JSON_STR/g;
                    let buttonProcessorString1 = buttonProcessorString.replace(re, json);
                    let real_url = window.URL.createObjectURL(new Blob([buttonProcessorString1], { type: 'text/javascript' }));
            ...
            this.node = new buttonNode(this.context, this.baseURL,
                {
            ...
            this.node.onprocessorerror = () => { console.log('An error from button-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createbuttonGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let buttonProcessorString = `
    ...
    class buttonProcessor extends AudioWorkletProcessor {
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            buttonProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, buttonProcessor.parse_item1);
            return params;
        }
...
const dspName = "button";

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.button = button;
    window.Faustbutton = button;
    window[dspName] = button;
} else {
